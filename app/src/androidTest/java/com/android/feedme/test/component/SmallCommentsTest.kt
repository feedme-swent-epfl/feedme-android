...
class SmallCommentsTest {
    ...
    fun checkCommentsDisplay() {
        ...
        // Testing the presence of the star icon
        composeTestRule.onNodeWithContentDescription("Recipe Image").assertIsDisplayed()

// Testing the display of the rating text
        composeTestRule.onNodeWithText(comment1.rating.toString()).assertIsDisplayed()
        composeTestRule.onNodeWithText(comment1.title).assertIsDisplayed()

// Waiting for the composition to become idle
        composeTestRule.waitForIdle()

// Testing the display of the comment title
        composeTestRule.onNodeWithText(comment1.title).assertIsDisplayed()
