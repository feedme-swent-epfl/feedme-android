...
fun EditProfileContent(
...

    {
        ...
        )
        {
            ...
            UserProfilePicture()
            Text(text ="Edit Picture", style = TextStyle(fontSize = 16.sp), modifier = Modifier.padding(top = 8.dp))
        }
        ...
        if (nameError != null) Text(nameError!!, color = MaterialTheme.colorScheme.error, style = TextStyle(fontSize = 12.sp))
        ...
        OutlinedTextField(
            ...
            onValueChange = {
                ...
                usernameError = when {
                    ...
                    !it.matches("^[\\w_]*$".toRegex()) -> "Username must be alphanumeric or underscores"
                    it.length < minNameSize -> "Username must be at least $minNameSize characters"
                    it.length > maxNameSize -> "Username must be no more than $maxNameSize characters"
                    else -> null
                }
            },
            label = { Text("Username") },
        ...
        if (usernameError != null) Text(usernameError!!, color = MaterialTheme.colorScheme.error, style = TextStyle(fontSize = 12.sp))

        OutlinedTextField(
            value = bio,
            onValueChange = {
                bio = it
                bioError = if (it.length > bioMaxSize) "Bio must be no more than $bioMaxSize characters" else null
            },
            label = { Text("Bio") },
        ...
        if (bioError != null) Text(bioError!!, color = MaterialTheme.colorScheme.error, style = TextStyle(fontSize = 12.sp))

        Button(
            onClick = {
                if (nameError == null && usernameError == null && name.isNotEmpty() && username.isNotEmpty()) {
            ...
            enabled = name.isNotEmpty() && username.isNotEmpty() && nameError == null && usernameError == null,
