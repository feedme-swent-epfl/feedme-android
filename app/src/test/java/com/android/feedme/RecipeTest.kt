...
class RecipeTest {
    ...
    fun setUp() {
        ...
        val recipe1 = Recipe(
            "1", "Pasta", "Delicious pasta", "Boil pasta, add sauce",
            ingredients = listOf(
                IngredientMetaData("1 tsp", "Teaspoon", ingredientSalt)
            ),
        ...
        val recipe2 = Recipe(
            "2", "Cake", "Sweet cake", "Mix ingredients, bake",
            ingredients = listOf(
                IngredientMetaData("2 cups", "Cup", ingredientSugar)
            ),
...
class RecipeProcessor {
    fun filterRecipesByIngredientType(recipes: List<Recipe>, type: String): List<Recipe> {
        return recipes.filter { recipe ->
            recipe.ingredients.any { it.ingredient.type == type }
        }
    }
}
