...
class DatabaseTest {
    ...
    fun setUp() {
        ...
        database = Database(mockFirestore)
    ...
    fun addComment_Success() {
        // Setup
        val comment = Comment("authorId", "recipeId", "photoURL", 5.0, 120.0, "Title", "Content", Date.from(
            Instant.now()))
        `when`(mockFirestore.collection("comments")).thenReturn(mockCollectionReference)
        ...
        database.addComment(comment, { successCalled = true }, { })
    ...
    fun getComment_Success() {
        ...
        val expectedComment = Comment("authorId", "recipeId", "photoURL", 5.0, 120.0, "Title", "Content",  Date.from(
            Instant.now()))
        ...
        // Execute & Verify
        database.getComment(commentId, { comment ->
